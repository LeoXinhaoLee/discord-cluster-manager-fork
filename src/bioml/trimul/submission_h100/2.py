...
def trimul_fused_projection(
    ...
    tl.store(l_output_block_ptr, l_proj_acc * l_gate_acc, boundary_check=(0, 1))
    tl.store(r_output_block_ptr, r_proj_acc * r_gate_acc, boundary_check=(0, 1))
    tl.store(g_output_block_ptr, o_gate_acc, boundary_check=(0, 1))
...
def triton_layernorm(
    ...
    for c in range(0, N, BLOCK_N):
        ...
        tl.store(out_block_ptr, x, boundary_check=(0, 1))
        ...
        if has_multiplier:
            mul_block_ptr = tl.advance(mul_block_ptr, (0, BLOCK_N))
...
class TriMulTriton(torch.nn.Module):
    ...
    def __init__(self, dim, hidden_dim):
        ...
        self.to_out = torch.nn.Linear(hidden_dim, dim, bias=False, dtype=torch.float32)
    ...
    def forward(self, x, mask):
        ...
        out = launch_triton_layernorm(
            out,
            self.to_out_norm_weight,
            self.to_out_norm_bias,
            g_out
        )
...
def custom_kernel(data: input_t) -> output_t:
    ...
    trimul.to_out_norm_weight = nn.Parameter(weights['to_out_norm.weight'].to(torch.float32))
    trimul.to_out_norm_bias = nn.Parameter(weights['to_out_norm.bias'].to(torch.float32))
